...
class ProggressiveLoading{
	...
	start(){
		...
		if(window.SamplingOfVisibilityFlag){//如果是可见度采样模式
			...
			var firstViewer=window.config.firstViewer
			window.c.position.set(firstViewer[0],firstViewer[1],firstViewer[2])
			window.c.rotation.set(firstViewer[3],firstViewer[4],firstViewer[5])
		...
		function loading(meshIndex) {//普通加载
			if(meshIndex.length>0)console.log(meshIndex)
		...
		function addModel2(m1,meshIndex,matrixConfig,structdesc0){
			// console.log("structdesc0",structdesc0)
...
function loadSubZip3_init_double (back){
	...
	window.loadSubZip3_worker.postMessage({//开始请求
		type:"start",
		"sceneName":window.param.projectName
	})
}
