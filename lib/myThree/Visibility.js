...
class Visibility{
    ...
    constructor(config) {
        ...
        this.socket=this.initSocket("localhost:8887")
    ...
    initSocket(socketURL){
        ...
        socket.on('list',  data=> {
            // console.log(data)
            scope.visualList[parseInt(data.index)]=data.list
					...
					if(scope.prePoint===point0){//如果视点没有移动
						if(first){//如果是第一次
                            var posIndex=this.getPosIndex(
                                scope.areaInf[0].min,
                                scope.areaInf[0].step,
                                scope.areaInf[0].max)
                            // console.log("this.visualList[posIndex]",posIndex,this.visualList[posIndex[3]])
                            if(this.visualList[posIndex[3]]){
                                // alert(1)
                                var list=this.visualList[posIndex[3]]
                                var list2=[]
                                for(var i=0;i<list.length;i++)
                                    if(!window.meshes[list[i]])
                                        list2.push(list[i])
                                // console.log("list2",list2)
                                scope.loading(list2)

                            }else scope.request(posIndex)
                            // for(var iii=0;iii<scope.areaInf.length;iii++){
    ...
    getPosIndex(min,step,max){
        ...
        var s=step
        var index=xi*s[1]*s[2]+yi*s[2]+zi
        return [xi,yi,zi,index]
    } 
    getList(min,step,max) {//用于渲染
        var scope=this
        var posIndex=getPosIndex(min,step,max)
        var xi=posIndex[0],yi=posIndex[1],zi=posIndex[2]
